...
class WDGBANDTI(nn.Module):
    def __init__(self, **config):
        super(WDGBANDTI, self).__init__()
...
class MolecularGCN(nn.Module):
    def __init__(self, in_feats, dim_embedding=128, padding=True, hidden_feats=None, activation=None):
        ...
        self.gnn2 = GCN(in_feats=dim_embedding, hidden_feats=hidden_feats, activation=activation)
        self.gnn3 = GCN(in_feats=dim_embedding, hidden_feats=hidden_feats, activation=activation)
        self.output_feats = hidden_feats[-1]
        self.output_transform = nn.Linear(self.output_feats*3, self.output_feats, bias=False)
    ...
    def forward(self, batch_graph):
        ...
        node_feats3 = self.gnn3(batch_graph, node_feats2)
        gcn_output_feats = torch.cat([node_feats1, node_feats2, node_feats3], dim=-1)
        batch_size = batch_graph.batch_size
        output_node_feats = gcn_output_feats.view(batch_size, -1, self.output_feats*3)
        output = self.output_transform(output_node_feats)
        return output
